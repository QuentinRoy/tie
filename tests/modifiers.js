...
test("Alter", assert => {
  const a = new Constraint(5);
  const b = a.alter(x => x + 4);
  assert.equal(b.get(), 9, "Simple alter has the proper value.");
  a.set(6);
  assert.equal(
    b.get(),
    10,
  ...
  c.set(2);
  assert.equal(d.get(), 12, "It is updated if this other constraint changes.");
  a.set(0);
  assert.equal(
    d.get(),
    6,
...
test("IfElse", assert => {
  ...
  const b = new Constraint("b");
  const cond = new Constraint(false);
  const res = cond.ifElse(a, b);
  assert.equal(res.get(), "b", "IfElse constriant gets the proper value.");
  cond.set(true);
  assert.equal(
    res.get(),
    "a",
...
test("Parse Numbers", assert => {
  const a = new Constraint("3");
  const b = new Constraint("3.6");
  const c = new Constraint("3.1px");
  ...
  assert.equal(af.get(), 3.0, "parseFloat is working for integer strings.");
  assert.equal(ai.get(), 3, "parseInt is working for integer strings.");
  a.set("7");
  assert.equal(
    af.get(),
    7.0,
  ...
  assert.equal(bf.get(), 3.6, "parseFloat is working for float strings.");
  assert.equal(bi.get(), 3, "parseInt is working for float strings.");
  b.set("49.1020");
  assert.equal(
    bf.get(),
    49.102,
  ...
  assert.equal(bi.get(), 49, "parseInt is properly updated for float strings.");
...
test("Type conversion", assert => {
  const base = new Constraint(0);
  const bool = base.toBoolean();
  const string = base.toStringConstraint();
  const num = base.toNumber();

  assert.equal(bool.get(), false, "toBoolean is working for falsy value");
  base.set("0");
  assert.equal(bool.get(), true, "toBoolean is working for truthy value");

  base.set({
    toString: () => "toString test"
  });
  assert.equal(string.get(), "toString test", "toStringConstraint is working.");

  base.set(true);
  assert.equal(num.get(), 1, "toNumber is working.");

  assert.end();
});

test("Other global JS functions", assert => {
  const c = new Constraint(7);
  const isNotANumber = c.isNaN();
  assert.notOk(isNotANumber.get(), "isNaN is false for number.");
  c.set(NaN);
  assert.ok(isNotANumber.get(), "isNaN is true for NaN.");
  c.set("4.9");
  assert.notOk(isNotANumber.get(), "isNaN is false for string number.");
  c.set("foo");
  assert.notOk(isNotANumber.get(), "isNaN is false for non number strings.");

  c.set(8 / 0);
  const finite = c.isFinite();
  assert.notOk(finite.get(), "isFinite is false for +Infinite number.");
  c.set(8);
  assert.ok(finite.get(), "isFinite is true for small number.");
  c.set(-2 / 0);
  assert.notOk(finite.get(), "isFinite is false for -Infinite number.");
...
test("Prop", assert => {
  ...
  const prop = new Constraint("p1");
  const c = a.prop(prop);
  assert.equal(c.get(), "p1-val", "Prop can be defined by a constraint.");
  prop.set("p3");
  assert.equal(
    c.get(),
    "p3-val",
  ...
  assert.equal(a.prop("p2", "p21").get(), "p21-val", "Multi-level prop works.");

  const subPropNum = new Constraint(1);
  const subPropName = new Constraint(() => `p2${subPropNum.get()}`);
  ...
  subPropNum.set(2);
  assert.equal(d.get(), "p22-val", "It is updated if this constraint changes.");
  assert.end();
});

test("Other operators that can be applied multiple times.", assert => {
  const a = new Constraint(8);
  const b = new Constraint(13);
  const c = new Constraint(0.5);
...
test("Equals and not equals", assert => {
  const a = new Constraint(6);
  const b = new Constraint(6);
...
test("Static constraint combination", assert => {
  const a = new Constraint(5);
  const b = new Constraint(2);

  const sum = Constraint.sum(a, b, 6);
  assert.equal(sum.get(), 5 + 2 + 6, "Constraint sum is working.");
  a.set(3);
  assert.equal(
    sum.get(),
    3 + 2 + 6,
  ...
  const product = Constraint.product(a, b, 2);
  assert.equal(product.get(), 3 * 2 * 2, "Constraint product is working.");
  b.set(10);
  assert.equal(
    product.get(),
    3 * 10 * 2,
  ...
  const max = Constraint.max(a, b, 11);
  assert.equal(max.get(), 11, "Constraint max is working.");
  a.set(20);
  assert.equal(
    max.get(),
    20,
  ...
  const min = Constraint.min(a, b, 0);
  assert.equal(min.get(), 0, "Constraint min is working.");
  b.set(-4);
  assert.equal(
    min.get(),
    -4,
  ...
  const hypot = Constraint.hypot(a, 8, b);
  assert.equal(
    hypot.get(),
    Math.hypot(20, -4, 8),
  ...
  a.set(90);
  b.set(15);
  const atan2 = Constraint.atan2(a, b);
  assert.equal(atan2.get(), Math.atan2(90, 15), "Constraint atan2 is working.");
  b.set(30);
  assert.equal(atan2.get(), Math.atan2(90, 30), "It is properly updated.");

  assert.end();
});

test("Unary operator", assert => {
  assert.notOk(new Constraint(true).not().get(), "Not true is not ok.");
  assert.ok(new Constraint(false).not().get(), "Not false is ok.");
  assert.notOk(new Constraint("thing").not().get(), "Not truthy is not ok.");
  assert.ok(new Constraint("").not().get(), "Not not truthy is ok.");

  assert.equal(new Constraint(5).neg().get(), -5, "Positive neg is correct.");
  assert.equal(new Constraint(-5).neg().get(), 5, "Negative neg is correct.");

  assert.equal(
    new Constraint(5).pos().get(),
    5,
    "Pos is working with numbers."
  );
  assert.equal(
    new Constraint("-28").pos().get(),
    -28,
    "Pos is working with strings."
  );

  assert.equal(
    new Constraint(2).bitwiseNot().get(),
    ~2, // eslint-disable-line no-bitwise
  ...
  assert.equal(
    new Constraint("6").pos().get(),
    6,
    "Pos is working for a string."
  );

  assert.equal(
    new Constraint([12]).pos().get(),
    12,
