...
export default class Constraint {
     ...
     * actually changed).
     */
    onMayHaveChanged(callback){
        this._mayHaveChangedListeners.push(callback);
    }

    /*
     * Removes a "may have changed" callback.
     */
    offMayHaveChanged(callback){
        const i = this._mayHaveChangedListeners.indexOf(callback);
        if(i >= 0){
    ...
    /*
     * Set all the constraints that depends on this constraint as
     * uncertain. Then recursively call this method on all these
     * constraints.
     */
    _setDescendantsUncertain(){
        this._dependentConstraints.forEach((dc) => {
            if(dc._state === valid){
                dc._setState(uncertain);
                dc._setDescendantsUncertain();
            }
        });
    }
    ...
    _setState(newState){
        if(this._state !== detached && newState !== this._state){
            const previous = this._state;
            this._state = newState;
            if(this._state === uncertain && previous !== invalid ||
               this._state === invalid && previous !== uncertain)
            {
                this._mayHaveChangedListeners.forEach((handler) => {
