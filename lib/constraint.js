...
/**
 * Golden rule: nothing gets out of this class without updating
...
export default class Constraint {
    ...
    _setDepsUncertain(){
        this._dependentConstraints.filter(
            dc => dc._state === valid
        ).forEach((dc) => {
    ...
    _addDependency(dep){
        ...
        dep._dependentConstraints.push(this);
    }
    ...
    _settle(){
        if(this._state === uncertain){
            ...
            // changes.
            const someParentChanged = this._dependencies.some((d) => d._update());
    ...
    _clearDependentConstraints(){
        this._dependentConstraints.forEach((d) => {
            // remove this from dependencies' dependent constraints
            const dx = d._dependencies.indexOf(this);
            if(dx >= 0) {
                d._dependencies.splice(dx, 1)
            }
        });
        // clear the dependencies
        this._dependentConstraints = [];
