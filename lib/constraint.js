 ...
 *   - as soon as one "unsettled" or "invalid" parent changed after its evaluation the constraint is
 *       re-evaluated. Because the dependencies are called in order of used, stopping as soon as one
 *       changed dependency is found ensure that we do not trigger the evaluation of a dependency that
 ...
 * has been invalidated but it is not sure yet that one of its parents has actually changed.
 * A constraint may still be in the "unsettled" state when all of its parent has been
 * re-evaluated without change but must not be in this state if we know (for sure) that one
...
/*
 * The constraint has been untied so that it can be garbaged collected.
...
export default class Constraint {
    ...
    constructor (getter, settings) {
        ...
        // Contains an object (see onChange).
        this._listeners = [];
        this._set(getter);
    }
    ...
    _unsettle(invalid=false, alreadyNotified=new Set()){
        ...
        // time _unsettle() is called without args.
        ...
        // If no process is currently in charge of the notification:
        // take the token.
        ...
        // The listener notifications are sent only once all dependent constraints have
        // been unsettled.
        // Using a global pending list of notifications also ensure that the notifications
        // added following constraint invalidation from previous notification are
        // inserted in the list, and called in the invalidation order.
        if(notifyListeners){
            // Do not cash pending notification's length as it may change during
            // the loop.
            for(let i=0; i < Constraint._pendingNotifications.length; i++){
                Constraint._pendingNotifications[i]._notifyListeners();
            }
     ...
     * constraint is unsettled (i.e. it has been notified that
     * that it may have changed as a result of one of its ancestor
     * being invalidated).
     *
     * @param check: check if the constraint has actually
     * changed before calling the callback.
     * This forces the constraint to be re-evaluted each time
     * it is unsettled.
     */
    onUnsettled(callback, checkChange=false){
        // FIXME: This is currently required to update the dependencies the first time but it
        // may not be required *each* time a new handler is added.
        // Also, it does not work if the constraint is re-set afterward.
        this._update();
     ...
     * Removes a listener.
     */
    offUnsettled(callback, checkChange){
        const i = this._listeners.findIndex(
            (entry) => entry.callback === callback && (
                checkChange == null || entry.check === checkChange
            )
        );
        if(i >= 0){
     ...
     *
     * Once the constraint is untied, its behavior is not guaranteed.
     */
    untie(){
        if(!this.untieable){
            // TODO: use a real exception instead of a string.
            throw "This constraint is not untieable.";
        }
        // Clear dependencies.
...
// List of constraints with with listeners whose notification is pending.
Constraint._pendingNotifications = [];
