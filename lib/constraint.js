...
export default class Constraint {
    ...
    /*
     * Add this constraint to the constraint stack, call f, then restore the previous
     * caller.
     */
    _stackCall(f){
     ...
     * Warning: registering a callback forces the constraint to be re-evaluated each times
     * it is unsettled!
     */
    onChange(callback){
        if(this._state === UNTIED){
     ...
     * useful to make sur the constraint memory can be de-allocated. This also condemn the constraint
     ...
     * a constraints tree is only guaranteed de-allocated when there is no outside reference anymore
