...
export default class Constraint {
    ...
    invalidate(){
        ...
        // Notify all constraints that depends on this constraint
        // that they may have change.
        // The use of a set makes sure that none of them is notified
        // more than once (see the losange test).
        const mayHaveChanged = new Set(this._dependentConstraints);
        mayHaveChanged.forEach((node) => {
            if(node._state !== invalid){
                node._setState(uncertain);
            }
            node._notifyListeners();
            node._dependentConstraints.forEach(
                dc => mayHaveChanged.add(dc)
            );
        });
    }
